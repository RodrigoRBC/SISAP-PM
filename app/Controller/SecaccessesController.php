...
class SecaccessesController extends AppController
{
	...
	{				
		...
		//pr($listaAros);
		$aros = $this->Menu->agregarTagArbol($listaAros,'Aro','etiqueta');
		//pr($aros);
		$this->set('aros',$aros);
	}
	...
	public function accederpermiso($aros_id) {
		...
		//pr($listaDeAccesos);
		$listaDeAccesos = (!empty($listaDeAccesos))?$this->Menu->agregarTagArbol($listaDeAccesos,'listaDeAccesos','etiqueta'):array();		
