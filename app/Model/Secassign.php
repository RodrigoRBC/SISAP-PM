...
class Secassign extends AppModel {
	...
	public $validate = array(
		...
		'status' => array('rule' => array('notBlank'))
	);
	...
	public function afterSave($created, $option=null) {
	...
	function licenciadiaactual(){
		$sql = "SELECT datediff(max(CURDATE()), min(fechainicio)) as dia FROM licencia";		
							
		$rs = $this->query($sql);
		
		$dia  = $rs[0][0];	

		return !empty($dia)?$dia:array();
	}


	function licenciadias(){
		$sql = "SELECT datediff(max(fechafin), min(fechainicio)) as dia FROM licencia";		
